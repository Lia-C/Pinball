...
public class SquareBumperTest {
    ...
    private static final double BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER = BALL_RADIUS
            / Math.sqrt(2.0);

    @BeforeClass
    public static void setUpBeforeClass() throws Exception {
        bumperInMiddle = new SquareBumper(HALF_OF_MAX_COORDINATE,
                HALF_OF_MAX_COORDINATE, new Gadget[0]);
        topLeftCornerBallCollideAtAngle = new Ball(
                new Vect(1, 1),
                new Geometry.DoublePair(
                        HALF_OF_MAX_COORDINATE
                                - BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER,
                        HALF_OF_MAX_COORDINATE
                                - BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER));
        topLeftCornerBallCollideHeadOn = new Ball(
                new Vect(1, 0),
                new Geometry.DoublePair(
                        HALF_OF_MAX_COORDINATE
                                - BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER,
    ...
    public final void testGetMinCollision() {
        assertTrue(Util.doublesAreEqual(bumperInMiddle
                .getMinCollisionTime(topLeftCornerBallCollideAtAngle), 0));
        assertTrue(Util.doublesAreEqual(bumperInMiddle
                .getMinCollisionTime(topLeftCornerBallCollideHeadOn), 0));
    ...
    public final void testinteractWithBallCornerAngle() {
        ...
        // defensive copying
        Ball topLeftCornerBallCollideAtAngleCopy = new Ball(new Vect(
                initialXVelocity, initialYVelocity), new Geometry.DoublePair(
                HALF_OF_MAX_COORDINATE
                        - BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER,
                HALF_OF_MAX_COORDINATE
                        - BALL_CENTER_TRANSLATION_AMOUNT_TO_BE_AT_CORNER));
        ...
        double finalXVelocity = topLeftCornerBallCollideAtAngleCopy
                .getVelocity().x();
        double finalYVelocity = topLeftCornerBallCollideAtAngleCopy
                .getVelocity().y();
    ...
    public final void testinteractWithBallLineSegment() {
        ...
        // defensive copying
        Ball leftSideBall = new Ball(new Vect(initialXVelocity,
