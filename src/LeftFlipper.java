 ...
 * 
 * Bounding box of size 2Lx2L The flipper's pivot point is in the northwest
 * corner of its bounding box, and its initial rotation is counterclockwise for
 * the default orientation (0 degrees). Requires that orientation is 0, 90, 180,
 * or 270. Coefficient of reflection: 0.95 Rotates 90 degrees when triggered.
 */
public class LeftFlipper implements Gadget {
    ...
    private final double ANGULAR_VELOCITY = 1080.0; // LeftFlippers always
    ...
    // represents a flipper that can rotate 90 degrees counterclockwise about
    // its
     ...
     * 
     * @param xCor
     *            x-coordinate of the upper-left corner of the bumper's bounding
     *            box must be in the range [0,19]
     * @param yCor
     *            y-coordinate of the upper-left corner of the bumper's bounding
     *            box must be in the range [0,19]
     * @param orientation
     *            rotation in degrees from the default orientation must be 0,
     *            90, 180, or 270
     */
    ...
    public boolean isOccupying(int x, int y) {
        if (orientation == 0) {
            if (isVertical) {
                if (x == xCor && y >= yCor && y <= yCor + 1) {
                    return true;
                } else {
                    return false;
                }
            } else {
                if (x >= xCor && x <= xCor + 1 && y == yCor) {
                    return true;
                } else {
                    return false;
                }
            ...
            throw new RuntimeException(
                    "isOccupying error, did not return true or false");
        }
    }
    ...
    public void Action() {
        ...
        if (angleRotated.compareTo(Angle.DEG_270) <= 0) {

            flipping = false;
            angleRotated = Angle.DEG_90;
            isVertical = !isVertical;
        }
        flipper = Geometry.rotateAround(flipper, pivot.getCenter(),
                angleRotated);
        endpoint = Geometry.rotateAround(endpoint, pivot.getCenter(),
                angleRotated);
        time = 0;
    }
    ...
    public void interactWithBall(Ball ball) {
        ...
        if (Util.getPartOfGadgetThatBallWillCollideWith(circles, lineSegments,
                ball) instanceof LineSegment) {
            LineSegment wall = (LineSegment) Util
                    .getPartOfGadgetThatBallWillCollideWith(circles,
                            lineSegments, ball);
            if (flipping) {
                newVel = Geometry.reflectRotatingWall(wall, pivot.getCenter(),
                        ANGULAR_VELOCITY, ball.getCircle(), ball.getVelocity(),
                        COEFFICIENT_OF_REFLECTION);
                ball.setVelocity(newVel);
            } else {
                newVel = Geometry.reflectWall(wall, ball.getVelocity(),
                        COEFFICIENT_OF_REFLECTION);
                ball.setVelocity(newVel);
            }
        } else if (Util.getPartOfGadgetThatBallWillCollideWith(circles,
                lineSegments, ball) instanceof Circle) {
            Circle corner = (Circle) Util
                    .getPartOfGadgetThatBallWillCollideWith(circles,
                            lineSegments, ball);
            if (flipping && corner.getCenter().x() == pivot.getCenter().x()
                    && corner.getCenter().y() == pivot.getCenter().y()) {
                // corner is pivot, is not moving
                newVel = Geometry.reflectCircle(corner.getCenter(), ball
                        .getCircle().getCenter(), ball.getVelocity(),
                        COEFFICIENT_OF_REFLECTION);
                ball.setVelocity(newVel);
            } else if (flipping) { // corner is endpoint, is moving
                newVel = Geometry.reflectRotatingCircle(endpoint,
                        pivot.getCenter(), ANGULAR_VELOCITY, ball.getCircle(),
                        ball.getVelocity(), COEFFICIENT_OF_REFLECTION);
                ball.setVelocity(newVel);
            } else { // not flipping
                newVel = Geometry.reflectCircle(corner.getCenter(), ball
                        .getCircle().getCenter(), ball.getVelocity(),
