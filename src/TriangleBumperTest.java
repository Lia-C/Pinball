...
public class TriangleBumperTest {
    /*
     * Testing Strategies: -Test all four orientations isOccupying: -True:
     * upper-left bounding box corner -False: outside interactWithBall: -ball
     * hits a corner at an angle -ball hits a line segment toString: -returns
     * "/" for orientation 0, 180 -returns "\" for orientation 90, 270
     * getPosition: -returns position trigger: -returns toTrigger
     * getMinCollisionTime: -returns top wall when approaching head-on from top
     * -when ball is touching this, minCollisionTime is 0 -when ball will not
     * collide with this, minCollisionTime is Double.POSITIVE_INFINITY
     */
    ...
    private static Ball hittingCorner; // Ball starting at (3,3) with velocity
                                       // (1,1)

    @BeforeClass
    public static void setUpBeforeClass() {
        bumper0 = new TriangleBumper(3, 3, new Gadget[] { new TriangleBumper(0,
                0, new Gadget[0]) });
        ...
        comingFromCorner = new Ball(new Vect(1.0, 1.0),
                new Geometry.DoublePair(1.0, 1.0));
    }
