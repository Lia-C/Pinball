...
/**
 * 
 * A mutable class representing the ball in the game. (mutable because we need
 * to set the velocity and position of the ball)
 *
 */
public class Ball {
    ...
    public void Action(Ball otherBall) {
        // TODO
        // use reflectBalls(Vect center1, double mass1, Vect velocity1, Vect
        // center2, double mass2, Vect velocity2)
    }
    ...
    public boolean isInBallSpace(double x, double y) {
        double distFromOrigin = Math.sqrt(Math.pow((x - this.position.d1), 2)
                + Math.pow((y - this.position.d2), 2));
